...
const NavBar = ({ current, onNavigate }: { current: Screen, onNavigate: (s: Screen) => void }) => (
  <nav className="fixed bottom-0 left-0 w-full flex justify-around items-end pt-2 pb-8 bg-surface/95 backdrop-blur-lg border-t border-outline-variant/20 shadow-[0_-4px_30px_rgba(51,51,51,0.03)] z-50 rounded-t-xl px-4">
    ...
    <button onClick={() => onNavigate(Screen.Profile)} className={`flex flex-col items-center justify-center transition-colors w-1/5 ${current === Screen.Profile ? 'text-primary' : 'text-secondary'}`}>
      <User size={20} className={current === Screen.Profile ? 'fill-current' : ''} />
  ...
  return (
    <header className="flex justify-between items-center px-4 py-2 w-full sticky top-0 z-50 bg-surface/80 backdrop-blur-md border-b border-outline-variant/30">
      <div className="flex items-center gap-4">
        ...
        <h1 className="font-display text-xl font-light text-primary tracking-[0.2em]">珍修</h1>
      </div>
    ...
    <div className="h-screen w-full flex flex-col items-center justify-center bg-surface overflow-hidden">
      ...
      >
        <div className="relative w-40 h-40 flex items-center justify-center">
          ...
          <div className="w-32 h-32 flex items-center justify-center bg-white rounded-full shadow-lg p-6 border border-outline-variant/20">
            <svg className="w-full h-full animate-kintsugi" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M20 40C20 20 80 20 80 40C80 70 65 90 50 90C35 90 20 70 20 40Z" stroke="#4D463A" strokeWidth="2" strokeLinecap="round"/>
              <path d="M35 25C35 25 45 40 40 55C35 70 55 85 55 85" stroke="#C9A96E" strokeWidth="2" strokeLinejoin="round" className="drop-shadow-[0_0_8px_rgba(201,169,110,0.8)]"/>
              <path d="M60 22L55 35L65 45L58 60L62 75" stroke="#C9A96E" strokeWidth="1.5" strokeLinecap="round" opacity="0.6"/>
            </svg>
          </div>
        </div>
        <motion.h1 
...
// 新增：首页功能导航回调类型
export type HomeFeatureCallback = (feature: 'evaluation' | 'studios' | 'commissions' | 'artisans') => void;

export const HomeScreen = ({ onScan, onNavigate, onFeatureNavigate }: { onScan: () => void, onNavigate: (s: Screen) => void, onFeatureNavigate?: HomeFeatureCallback }) => (
  <div className="bg-surface min-h-screen">
    ...
    <main className="pb-32">
      ...
      {/* Quick Access Section - 四个功能入口 */}
      <section className="px-4 mt-8 mb-4">
        <div className="grid grid-cols-4 gap-4">
          {[
            { icon: BarChart2, label: '在线评估', feature: 'evaluation' as const },
            { icon: Mail, label: '到店寄修', feature: 'studios' as const },
            { icon: FileText, label: '我的委托', feature: 'commissions' as const },
            { icon: Users, label: '匠人名录', feature: 'artisans' as const }
          ].map((item, i) => (
            <button 
              key={i} 
              className="flex flex-col items-center gap-2 group"
              onClick={() => onFeatureNavigate?.(item.feature)}
            >
      ...
      <section className="mt-8">
        ...
        <div className="flex overflow-x-auto gap-5 px-4 hide-scrollbar snap-x pb-2">
          {CASES.map((item) => (
  ...
  <div className="h-screen w-full bg-black overflow-hidden relative">
    ...
    <div className="relative z-10 h-full flex flex-col">
      ...
      <header className="flex justify-between items-center px-4 py-4 text-white">
        ...
        <h1 className="font-display text-xl tracking-[0.2em]">珍修</h1>
...
export const ProfileScreen = ({ onNavigate }: { onNavigate: (s: Screen) => void }) => (
  <div className="bg-surface min-h-screen text-[#5D4037] pb-24">
    {/* Top App Bar */}
    <header className="flex justify-between items-center px-4 h-16 w-full fixed top-0 z-50 bg-surface/80 backdrop-blur-md">
      <button 
        onClick={() => onNavigate(Screen.Home)}
        className="flex items-center justify-center p-2 active:scale-95 transition-transform"
      >
        <ArrowLeft className="text-[#5D4037]" size={24} />
      </button>
      <div className="flex items-center gap-2">
        <button className="flex items-center justify-center p-1.5 border border-primary-container rounded-full active:scale-95 transition-transform">
          <Headset className="text-primary-container" size={20} />
        </button>
        <button className="flex items-center justify-center p-1.5 border border-primary-container rounded-full active:scale-95 transition-transform">
          <Settings className="text-primary-container" size={20} />
        </button>
      </div>
    </header>
    ...
    <main className="mt-20 px-4 space-y-6">
      ...
      <section className="flex flex-col gap-4">
        <div className="flex items-center gap-4">
          <div className="relative">
            <img 
              alt="User Profile" 
              className="w-16 h-16 rounded-full border border-primary-container p-0.5 object-cover" 
              src="https://lh3.googleusercontent.com/aida-public/AB6AXuClSHw8ZsqEkanb-e1sSyqT8saHOo214nFBC38Hy6jSMFEQ5SlD-wJiM-tXXj8LkhOLbt4-_vmZIaoP-86Jq9K0giECnaGfAwcwRON1uxw03JUNuqO7ht1dFlO7oiMMe2c3-WGxiF6Zzjtp924p_Ixod2Hb5dSZkXip3WPR1t6fT-tqsxtM9rGTyfdtOaBls8iJ9fxVStNOv7mEtCZSDjIJsTqmU95p1GYy-XpaoOSwFfKHegXY7sEkKsNjAvutMP-4KnO006PAYUo" 
            />
          </div>
          <div className="flex flex-col">
            <h2 className="font-display text-xl font-medium text-[#5D4037] flex items-center gap-1">
              珍修会员 <span className="text-primary-container text-sm tracking-tighter">★★★★★</span>
            </h2>
            <button className="text-label text-primary-container mt-1 text-left">会员中心 &gt;</button>
          </div>
        </div>
        <div className="space-y-1">
          <div className="flex justify-between items-center text-[11px] text-[#5D4037]/60 tracking-wider">
            <span>成长值 12308</span>
            <span>30000</span>
          </div>
          <div className="h-1 w-full bg-primary-container/10 rounded-full overflow-hidden">
            <div className="h-full bg-gradient-to-r from-[#C9A96E] to-[#E4C285] w-[41%]"></div>
          </div>
        </div>
      </section>
      ...
      <section className="grid grid-cols-4 gap-3">
        {[
          { icon: Wrench, label: '珍修工具', tag: '速领' },
          { icon: Smartphone, label: '潮玩资讯', tag: '限时' },
          { icon: Ticket, label: '每日领券', tag: '每日' },
          { icon: Palette, label: '匠人手工', tag: '精品' }
        ].map((item, i) => (
          <div key={i} className="bg-primary/5 rounded-2xl p-3 flex flex-col items-center justify-between aspect-square border border-primary-container/5 relative overflow-hidden">
            <span className="absolute top-0 right-0 bg-primary-container text-white text-[8px] px-1.5 py-0.5 rounded-bl-lg">
              {item.tag}
            </span>
            <div className="flex-1 flex items-center justify-center">
              <item.icon size={24} className="text-[#5D4037]" />
            </div>
      ...
      {/* Quick Access Card */}
      <section className="bg-white/60 backdrop-blur-sm rounded-2xl border border-[#5D4037]/10 p-4 space-y-6">
        {/* Row 1 */}
        <div className="grid grid-cols-4 gap-4">
          {[
            { icon: Bookmark, label: '收藏' },
            { icon: History, label: '浏览记录' },
            { icon: Wallet, label: '红包卡券' },
            { icon: Coins, label: '珍修币' }
          ].map((item, i) => (
            <div key={i} className="flex flex-col items-center gap-1.5">
              <item.icon className="text-primary-container" size={20} />
              <span className="text-[11px]">{item.label}</span>
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-4 gap-4 pt-2">
          {[
            ...
            { icon: MessageSquareText, label: '待评价' },
            { icon: Undo2, label: '退款售后' }
          ].map((item, i) => (
            <div key={i} className="flex flex-col items-center gap-1.5">
              <item.icon className="text-primary-container" size={20} />
              <span className="text-[11px]">{item.label}</span>
      ...
      {/* Wallet & Billing */}
      <section className="bg-white/60 backdrop-blur-sm rounded-2xl border border-[#5D4037]/10 px-4">
        <div className="py-4 flex justify-between items-center border-b border-primary-container/10 px-1">
          <div className="flex items-center gap-3">
            <CreditCard className="text-primary-container" size={20} />
            <span className="text-sm">进入钱包</span>
          </div>
          <ChevronRight className="text-[#5D4037]/40" size={18} />
        </div>
        <div className="py-4 flex justify-between items-center px-1">
          <div className="flex items-center gap-3">
            <Calendar className="text-primary-container" size={20} />
            <span className="text-sm">珍修月付</span>
          </div>
          <div className="flex items-center gap-1">
            <span className="text-[11px] text-primary-container">本月省￥28</span>
            <ChevronRight className="text-[#5D4037]/40" size={18} />
          </div>
        </div>
      </section>
      ...
      <section className="space-y-4">
        <h3 className="text-[#5D4037] font-medium px-1 flex items-center gap-2">
          <span className="w-1 h-4 bg-primary-container rounded-full"></span>
          珍修神券 · 每日可领
        </h3>
        <div className="grid grid-cols-4 gap-2">
          {[
            { price: '13', desc: '满100可用' },
            { price: '10', desc: '满80可用' },
            { price: '5', desc: '无门槛' },
            { price: '20', desc: '大额专享' }
          ].map((c, i) => (
            <div key={i} className="bg-white border border-primary-container/20 rounded-xl p-2 text-center flex flex-col items-center">
              <span className="text-primary-container font-bold text-lg">￥{c.price}</span>
              <span className="text-[9px] text-[#5D4037]/60">{c.desc}</span>
            </div>
          ))}
        </div>
      </section>
    ...
    <NavBar current={Screen.Profile} onNavigate={onNavigate} />
  </div>
);
