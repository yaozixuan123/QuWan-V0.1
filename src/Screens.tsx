...
import { Screen, CaseStudy, Scheme } from './types';
...
export const HomeScreen = ({ onScan, onNavigate }: { onScan: () => void, onNavigate: (s: Screen) => void }) => {
  const { status, data, error } = useCatalogContext();
  const cases: CaseStudy[] = data?.caseStudies ?? [];
    ...
    <main className="pb-32">
      ...
      <section className="mt-8">
        ...
        <div className="flex overflow-x-auto gap-stack-gap-md px-4 hide-scrollbar snap-x pb-2">
          {status === 'loading' && (
            <div className="min-w-[280px] snap-start rounded-xl border border-outline-variant/10 bg-white/60 p-8 text-center text-xs text-secondary">
              正在从服务器加载案例…
            </div>
          )}
          {status === 'error' && (
            <div className="min-w-[280px] snap-start rounded-xl border border-red-200 bg-red-50/80 p-4 text-xs text-red-800">
              {error ?? '加载失败'}（请确认已运行 <code className="font-mono">npm run dev:api</code>）
            </div>
          )}
          {cases.map((item) => (
  ...
  <div className="bg-surface min-h-screen">
    ...
    <main className="px-4 pt-10 pb-40 max-w-6xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {status === 'loading' && (
          <div className="col-span-full rounded-xl border border-outline-variant/20 bg-white/70 p-8 text-center text-sm text-secondary">
            正在加载方案数据…
          </div>
        )}
        {status === 'error' && (
          <div className="col-span-full rounded-xl border border-red-200 bg-red-50/90 p-4 text-sm text-red-800">
            {error ?? '加载失败'}
          </div>
        )}
        {schemes.map((scheme) => (
    ...
    <NavBar current={Screen.Scheme} onNavigate={onNavigate} />
  </div>
  );
};
